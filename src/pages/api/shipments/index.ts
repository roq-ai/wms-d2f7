...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getShipmentsPublic() {
    ...
    const findOptions = convertQueryToPrismaUtil(query, 'shipment');
    const countOptions = omit(findOptions, 'include');
    const [totalCount, data] = await prisma.$transaction([
      prisma.shipment.count(countOptions as unknown),
  ...
  async function getShipments() {
    ...
    const limit = parseInt(_limit as string, 10) || 20;
    const offset = parseInt(_offset as string, 10) || 0;
    const response = await prisma.shipment
      .withAuthorization({
        roqUserId,
      ...
      .findManyPaginated({
        ...convertQueryToPrismaUtil(query, 'shipment'),
        take: limit,
        skip: offset,
        ...(order?.length && {
          orderBy: getOrderByOptions(order),
