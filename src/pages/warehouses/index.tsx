...
type ColumnType = ColumnDef<WarehouseInterface, unknown>;
...
export function WarehouseListPage(props: WarehouseListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getWarehouses({
        ...
        order: params.order,
        searchTermKeys: ['location.contains', 'warehouse_type.contains', 'operational_hours.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<WarehouseInterface>>(
    () => `/warehouses?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: WarehouseInterface) => {
    if (hasAccess('warehouse', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/warehouses/view/${row.id}`);
    }
  };
  ...
  const columns: ColumnType[] = [
    ...
    { id: 'manager_id', header: 'manager_id', accessorKey: 'manager_id' },
    { id: 'warehouse_type', header: 'Warehouse Type', accessorKey: 'warehouse_type' },
    { id: 'operational_hours', header: 'Operational Hours', accessorKey: 'operational_hours' },
    !hideActions
      ? {
          id: 'actions',
          header: '',
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="warehouse" />
        </Flex>

        {hasAccess('warehouse', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/warehouses/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
