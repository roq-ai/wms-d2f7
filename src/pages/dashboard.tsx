...
import { getProductCategories } from 'apiSdk/product-categories';
...
function DashboardPage() {
  ...
  const productCategoryStat = useSWR<PaginatedInterface<ProductCategoryInterface>>(
    () => `/product-categories`,
    () => getProductCategories({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    },
    {
      ...
      result: productCategoryStat?.data?.totalCount,
      isLoading: productCategoryStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
